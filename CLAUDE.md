...
- **`djomi-sweep` closes the "merchant pays but never returns to the tab" gap** — the one case neither the poll nor the (dormant) webhook covers on their own. Cron job (hourly, not yet scheduled): re-checks every `djomi_pending_payments` row with `resolved_at IS NULL AND transaction_id IS NOT NULL`, bounded to the last 24h (`listUnresolvedPendingPayments` in `_shared/djomi.ts`) so a genuinely abandoned attempt from days ago doesn't get polled against Djomi forever. Same `x-cron-secret`/`CRON_SECRET` auth pattern as `send-alpha-quota-reminders`.
...
- **Not yet built**: the proactive WhatsApp outreach that tells a merchant this page exists (a trial-ending / quota-exhausted reminder via the existing Twilio WhatsApp integration, sent by a backend job — never triggered by an in-app tap, for the same compliance reason `djomi-checkout` itself is never linked in-app). Until that exists, this path has no real acquisition channel.
...
- **Branded URL: `functions/abonnement.js` (repo root, Cloudflare Pages Functions — separate from `supabase/functions/`) transparently proxies `https://patron.kolilink.com/abonnement` to `djomi-checkout`**, so the link shared with merchants isn't a raw `*.supabase.co` URL. It's a proxy (streams the response through), not a redirect, so the address bar never changes and the checkout page's own same-origin `fetch(location.pathname, ...)` calls (payment creation, `?verify=1&ref=...` polling) keep working unmodified. `patron.kolilink.com` is already a live Cloudflare Pages deployment of this same repo — confirmed via the committed `public/_redirects` (SPA catch-all) and the root-level `privacy.html`/`support.html` it also serves; Cloudflare Pages auto-detects a `functions/` directory at the repo root with no dashboard config needed. `functions/` is plain `.js`, not `.ts`, and excluded from `tsconfig.json` for the same reason `supabase/functions` is — different runtime, different globals (`PagesFunction` isn't part of this project's type setup). **This only goes live once deployed on whatever branch Cloudflare Pages treats as production** (typically `main`) — pushing to a feature branch alone may only produce a preview deployment, not update the real domain.
