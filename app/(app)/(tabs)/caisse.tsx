    ...
    <Modal visible={visible} animationType="slide" presentationStyle="formSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.modalSafe}>
        ...
        <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
          ...
          {/* Client picker — shown for all methods */}
          {method === 'credit' ? (
            <View>
...
function SuccessOverlay({ visible, total, currency, onClose }: SuccessOverlayProps) {
  return (
    <Modal visible={visible} animationType="fade" transparent>
      <View style={styles.successBg}>
        <Card style={styles.successCard} elevated>
          <Text style={styles.successIcon}>✅</Text>
          <Text variant="h3" style={{ textAlign: 'center' }}>Vente enregistrée</Text>
          <Text variant="amountLarge" style={{ color: palette.success, textAlign: 'center' }}>
            {formatAmount(total, currency)}
          </Text>
          <Button label="Nouvelle vente" onPress={onClose} fullWidth size="lg" style={{ marginTop: spacing[2] }} />
        </Card>
      </View>
    </Modal>
  );
  ...
  return (
    ...
    >
      ...
      <Text variant="caption" color="secondary" numberOfLines={1}>
        {outOfStock ? '⚠️ Rupture' : `${product.stock_qty} ${product.unit}`}
      </Text>
...
export default function CaisseScreen() {
  ...
  const { cart, submitting, addToCart, removeFromCart, setQty, toggleBulk, clearCart, submitSale, clearError } =
    useSalesStore();
  ...
  const handleConfirmPayment = useCallback(
    ...
    [businessId, userId, cartTotal, submitSale, fetchProducts],
  );
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      {products.some(p => p.bulk_price) && (
        <View style={styles.hintBanner}>
          <Text variant="caption" style={{ color: colors.warning[700] }}>
            💡 Appuyez longuement sur un produit GROS pour ajouter en vente de gros
          </Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  successCard: { width: '100%', alignItems: 'center', gap: spacing[3] },
