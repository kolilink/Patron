-- ============================================================
-- Patron — Migration v101
-- Run in Supabase SQL Editor AFTER migration_v100
--
-- Links shipping expenses to the purchase order that created
-- them. Adds purchase_order_id to expenses and updates
-- receive_purchase_order to populate it.
--
-- Impact on existing data: NULL (no back-fill needed — old
-- expenses still appear in Dépenses, just not linked to a PO).
-- Going forward every auto-created transport_achat expense
-- carries the PO id so the PO detail screen can show it.
-- ============================================================

-- 1. Add the column (nullable FK so old rows are unaffected)
ALTER TABLE expenses
  ADD COLUMN IF NOT EXISTS purchase_order_id uuid
  REFERENCES purchase_orders(id) ON DELETE SET NULL;

-- 2. Index for the PO detail query
CREATE INDEX IF NOT EXISTS expenses_purchase_order_id_idx
  ON expenses(purchase_order_id)
  WHERE purchase_order_id IS NOT NULL;

-- 3. Recreate receive_purchase_order — identical to v88 except
--    the transport expense INSERT now includes purchase_order_id
