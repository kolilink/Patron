-- ============================================================
-- Patron — Migration v104
-- Run in Supabase SQL Editor AFTER migration_v103
--
-- Fixes three false-positive sources in run_reconciliation()
-- (added in v99) discovered after the first manual run on
-- 2026-06-30 — 32 of 41 critical findings were checker bugs,
-- not real data corruption:
--
--   Check #1 & #2 (Stock) — did not exclude is_system products
--     (e.g. "Solde reporté", the carnet-debt carryover placeholder
--     created by submit_carnet_debt). It never has stock_moves by
--     design — it isn't a real, physically-stocked item.
--
--   Check #2 (Stock) — compared a SINGLE so_line's qty against the
--     SUM of stock_moves for that product across the WHOLE order.
--     Any order with 2+ lines of the same product (e.g. one cart
--     entry at qty 10, another at qty 5 for the same product)
--     failed every line even though the totals matched exactly.
--     Fixed by aggregating so_lines by (order_id, product_id)
--     before comparing.
--
--   Check #9 (Ventes) — compared total_amount against
--     SUM(qty * unit_price), where unit_price is always the
--     catalogue price. When a sale is recorded above catalogue
--     price (vendre.tsx: payment amount exceeds cart total),
--     the real price is in unit_price_paid, not unit_price.
--     Fixed by summing qty * COALESCE(unit_price_paid, unit_price).
--
-- Only checks #1, #2, #9 changed. Checks #3–#8 and #10–#68 are
-- byte-for-byte identical to v99. Safe to re-run.
-- ============================================================
...
BEGIN
  ...
  --    [v104] Excludes is_system products (e.g. "Solde reporté") — they
  --    are not physically stocked and never get a stock_move by design.
  ...
  --    [v104] Aggregates so_lines by (order_id, product_id) before
  --    comparing — a sale with multiple lines for the same product
  --    (e.g. 10 units + 5 units in separate cart entries) must compare
  --    the SUM of those lines (15) against the stock_moves total, not
  --    each line individually. Also excludes is_system products.
  INSERT INTO reconciliation_findings
    (run_id,check_id,domain,check_name,severity,business_id,entity_type,entity_id,detail,affected_count)
  ...
  --    [v104] Sums qty * COALESCE(unit_price_paid, unit_price) instead
  --    of plain unit_price. unit_price is always the catalogue price;
  --    when a sale is recorded above catalogue price (vendre.tsx:
  --    payment exceeds cart total), the real price-per-unit lives in
  --    unit_price_paid. Comparing against unit_price alone flagged
  --    every above-catalogue sale as a mismatch.
  INSERT INTO reconciliation_findings
    (run_id,check_id,domain,check_name,severity,business_id,entity_type,entity_id,detail,affected_count)
