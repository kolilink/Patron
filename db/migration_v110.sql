-- ============================================================
-- Patron — Migration v110
-- Run in Supabase SQL Editor AFTER migration_v109
--
-- get_reports_snapshot(): canonical backend formula for the
-- Rapports screen. Replaces all client-side JS arithmetic.
--
-- Fixes three bugs present in the previous client-side approach:
--
--   1. Period attribution: uses sale_date (when the sale was made)
--      instead of payments.date (when cash was collected). A credit
--      sale made in January now correctly appears in January profit,
--      not in February when the client finally pays.
--
--   2. Stock losses (perte): cost of lost/damaged inventory is now
--      subtracted from gross profit. Previously invisible.
--
--   3. Cash on hand: computes the all-time position (all inflows
--      minus all outflows) instead of period cash flow. The number
--      no longer changes when you toggle "mois" vs "trimestre".
--
-- The function is role-aware: admin/manager see the full business
-- view; vendeur sees only their personal sales; investisseur sees
-- the business health + their personal investment stake.
--
-- Called by the app with p_today = device local date (same pattern
-- as get_dashboard_kpis v102) to avoid UTC midnight drift.
-- ============================================================
...
BEGIN
  ...
  IF p_role IN ('administrateur', 'manager', 'investisseur') THEN
    ...
    SELECT COALESCE(SUM(
      sm.qty * COALESCE(pv.cost_price, p.cost_price, 0)
    ), 0)
    INTO v_stock_losses
    FROM stock_moves sm
