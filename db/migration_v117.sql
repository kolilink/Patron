-- ============================================================
-- Patron — Migration v117
-- Run in Supabase SQL Editor AFTER migration_v116
--
-- Fix: get_reports_snapshot() (added by migration_v110) has been
-- raising "column sm.variant_id does not exist" on every single
-- call since it was deployed — the stock-losses subquery joins
-- product_variants on sm.variant_id, but stock_moves has never had
-- a variant_id column (stock_moves rows, including 'perte' losses,
-- are always product-level only; only so_lines / purchase_order_lines
-- carry variant_id). This made the Rapports screen show no data for
-- every business, for every role.
--
-- Fix: drop the bogus join and use the product's own cost_price,
-- matching how stock_moves has always been recorded.
-- ============================================================
