-- ============================================================
-- Patron — Migration v51
-- Run in Supabase SQL Editor AFTER migration_v50
--
-- Fix 1: submit_sale — stock deduction is no longer best-effort.
--   Previously, a stock INSERT/UPDATE failure was silently swallowed
--   (EXCEPTION WHEN OTHERS THEN NULL). The sale went through but stock
--   was never decremented — invisible inventory corruption.
--   Now any stock error rolls back the entire sale transaction.
--
-- Fix 2: memberships DELETE policy — last admin cannot leave their
--   own business. Previously any admin could delete their own row,
--   leaving the business without an admin and unmanageable.
-- ============================================================


-- ─── Fix 1: submit_sale (based on v42 client_id-aware version) ──────────────
-- Drop all existing overloads before recreating — Postgres won't let us rename
-- parameters with CREATE OR REPLACE.
DROP FUNCTION IF EXISTS public.submit_sale(uuid,uuid,text,date,numeric,numeric,boolean,jsonb,text,numeric,text);
DROP FUNCTION IF EXISTS public.submit_sale(uuid,uuid,text,date,numeric,numeric,boolean,jsonb,text,numeric,text,uuid);
...
BEGIN
  ...
  -- 4. Stock deduction — failure rolls back the entire sale (no silent swallow).
  --    If a product no longer exists at submit time, the merchant sees an error
  --    rather than a sale with phantom stock.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_cart) LOOP
...
-- Grant all callers — single canonical signature going forward
...
-- ─── Fix 2: last-admin cannot leave their own business ───────────────────────

-- SECURITY DEFINER helper — avoids RLS recursion when the policy itself
-- queries the memberships table (same pattern as v32's has_manager helper).
CREATE OR REPLACE FUNCTION count_business_admins(p_business_id uuid)
RETURNS int
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
...
GRANT EXECUTE ON FUNCTION count_business_admins(uuid) TO authenticated;

-- Tighten the DELETE policy: block a sole admin from deleting their own row.
-- An admin with a co-admin can still leave. A manager can remove anyone else.
