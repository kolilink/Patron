-- ============================================================
-- Patron — Migration v81
-- Run in Supabase SQL Editor AFTER migration_v80
--
-- Fix: snapshot cost_price at sale time.
--   so_lines previously had no cost column — get_order_cogs
--   joined to the *current* products.cost_price, meaning every
--   receipt of a new batch retroactively changed historical COGS
--   and profit figures. This migration:
--     1. Adds cost_price_at_sale (BIGINT cents) to so_lines.
--     2. Recreates submit_sale to snapshot the cost into each
--        so_line at the moment of sale (variant price preferred
--        over parent product price, matching the POS logic).
--   Historical rows keep cost_price_at_sale = NULL; migration_v82
--   updates get_order_cogs to fall back to current cost for those.
-- ============================================================

-- ─── 1. New column ───────────────────────────────────────────────────────────

ALTER TABLE so_lines
  ADD COLUMN IF NOT EXISTS cost_price_at_sale BIGINT;
...
BEGIN
  ...
  -- Insert so_lines with cost snapshot
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_cart) LOOP
    -- Resolve cost: variant preferred, fall back to product
    v_cost_price := NULL;
